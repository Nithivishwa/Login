...
def validate_email (username, email):
    with UserDataBase() as db:
        print(db.validate_email(username, email))
        
def help():
    print('Usage: %s { create | login | change-password | email | help }'
           % os.path.basename(sys.argv[0]))


if __name__ == '__main__':
    actions = {'create': create_user, 'login': login,
               'change-password': change_password, 'email': validate_email, 'help': help}
    try:
        action = sys.argv[1]
    except IndexError:
        action = 'help'
